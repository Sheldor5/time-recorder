...
public class RuleLoaderTest {
  ...
  public void test_apply_day() throws IOException {
    ...
    end = new Record(monday, LocalTime.of(22, 1), RecordType.CHECKOUT);
    session = new Session(monday, begin, end);
    day.addItem(session);

    begin = new Record(monday, LocalTime.of(22, 16), RecordType.CHECKIN);
    end = new Record(monday, LocalTime.of(23, 0), RecordType.CHECKOUT);
    session = new Session(monday, begin, end);
    day.addItem(session);
    ...
    Assert.assertEquals("Session summary should be 7200 seconds (5 hours)", 18000L, sessions.get(4).getSummary());
    Assert.assertEquals("Session valued summary should be 7200 seconds (5 hours)", 18000L, sessions.get(4).getValuedSummary());
    ...
    Assert.assertEquals("Session summary should be 2640L seconds (44 minutes)", 2640L, sessions.get(6).getSummary());
    Assert.assertEquals("Session valued summary should be 3960 seconds (1 hours, 28 minutes)", 3960L, sessions.get(6).getValuedSummary());

    Assert.assertEquals("Day summary should be 62100L seconds (17 hours, 15 minutes)", 62100L, day.getSummary());
