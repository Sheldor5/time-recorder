...
public class BeforeTest {
  ...
  public void test_before() {
    ...
    Assert.assertTrue(actual.getBegin().equals(time));

    Assert.assertEquals("Failed to update session", 1L * TimeUtils.HOUR_IN_MILLIS, session.getDuration());
    Assert.assertEquals("Failed to update session", (long) (1.5D * TimeUtils.HOUR_IN_MILLIS), session.getValuedDuration());

    Assert.assertEquals("Failed to update session", 18L * TimeUtils.HOUR_IN_MILLIS, actual.getDuration());
    Assert.assertEquals("Failed to update session", 18L * TimeUtils.HOUR_IN_MILLIS, actual.getValuedDuration());
  }
}