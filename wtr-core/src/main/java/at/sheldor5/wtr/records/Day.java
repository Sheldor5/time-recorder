...
public class Day extends Container<Record> {
  ...
  public final long getSummary() {
    ...
    if (records.get(0).getType() == RecordType.CHECKOUT) {
      records.add(0, new Record(Record.getStartOfDay(records.get(0).getTimestamp()), RecordType.CHECKIN));
    }

    if (records.get(records.size()-1).getType() == RecordType.CHECKIN) {
      records.add(records.size(), new Record(Record.getEndOfDay(records.get(records.size()-1).getTimestamp()), RecordType.CHECKOUT));
    }
